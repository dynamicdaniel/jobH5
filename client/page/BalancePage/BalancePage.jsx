...
class BalancePage extends Component {
    ...
    componentDidMount(){
        ...
        const height1 = ReactDOM.findDOMNode(this.refs.listview)
        console.log(height1)
        this.setState({
            bodyHeight:height,
            show:'visible'
          ...
          //  console.log(params)
            ...
            getBundingdevice(params1).then(res=>{
            ...
            <Bk height={`${this.state.bodyHeight}px`} overflowY='auto' visibility={this.state.show} bgColor='#f2f2f2'>
                ...
                <Bk height='28%'
                    ...
                    <Bk height='35%' display='flex' justifyContent='center'>
                        <Bk width='73.45%'>
                            <Bk width='40%' display='flex' alignItems='center'>
                                <Bk 
                                onClick={()=>{
                                    console.log('进入充值卡')
                                    scanCode().then(res=>{
                 
                                        getEntityDetail({cardCode : res}).then(cardDetail=>{
                                            console.log(cardDetail)
                                            this.setState({
                                                entityCardShow : true,
                                                entityCardDetail : cardDetail
                                            },()=>{
                                                console.log('扫码结果：',this.state.entityCardDetail)
                                            })      
                            ...
                            <Bk width='60%' display='flex' justifyContent='flex-end' alignItems='center' color='white'
                                ...
                                if( Object.keys(this.state.chongzhiItem).length > 0 ){
                                    ...
                                    window.wx.miniProgram.getEnv(res=>{
                                        if(res.miniprogram == true){
                                            ...
                                            const url = `pay?type=1&token=${this.token}&shopId=${this.shopId}&cardId=${this.cardId}&storeId=${storeId}&deviceNo=${deviceNo}&amountId=${this.state.chongzhiItem.id}`
                                    ...
                                    if(reg.test(this.state.payMoney)){
                                        window.wx.miniProgram.getEnv(res=>{
                                            ...
                                            if(res.miniprogram == true){
                                                const storeId = this.state.pickerData[this.state.pickerValue[0]].storeId
                                                const deviceNo = this.state.pickerData[this.state.pickerValue[0]].deviceNo
                                                const url = `pay?type=2&token=${this.token}&shopId=${this.shopId}&cardId=${this.cardId}&storeId=${storeId}&deviceNo=${deviceNo}&amount=${this.state.payMoney}`
                                                window.wx.miniProgram.navigateTo({
                                                    url: url
                                                })
                                            }else{
                                        ...
                                        Object.keys(this.state.selectItem).map((key,index)=>{
                                            if(index < 2){
                                                ...
                                                if (this.state.selectItem && this.state.selectItem[key] && this.state.selectItem[key].length > 0){
                                                    return ( 
                                                        <Dv key={index} height='auto' display="flex" flexWrap='wrap' alignItems='flex-start' color="#434343" fontSize="1em" marginTop='3px'>
                                                            <span>{index + 1}. 赠送卡券:</span>
                                                            <ul style={{margin: "0", padding: "0 16px", listStyle: "none"}}>
                                                            {
                                                                this.state.selectItem[key].map((item, child_index) => {
                                                                    return <li key={child_index}>
                                                                        <span >{`${item.title}:${item.num}张`}</span>
                                                                    </li>
                                                                })
                                                            }
                                                            </ul>
                                                ...
                                                if (this.state.selectItem && this.state.selectItem[key] && this.state.selectItem[key].length > 0) {
                                                    return (
                                                        <Dv key={index} height='auto' display="flex" flexWrap='wrap' alignItems='flex-start' color="#434343" fontSize="1em" marginTop='3px'>
                                                            <span>{index + 1}. 赠送次卡:</span>
                                                            <ul style={{margin: "0", padding: "0 16px", listStyle: "none"}}>
                                                            {
                                                                this.state.selectItem[key].map((item, child_index) => {
                                                                    if (item.num > 0) {
                                                                        return <li key={child_index}>
                                                                            <span >{`${item.title}:${item.num}次`}</span>
                                                                        </li>;
                                                                    }
                                                                    return 
                                                                })
