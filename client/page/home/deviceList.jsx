...
class DeviceList extends Component {
    renderItem = (item) => {
        let check = false
        if (item.deviceNo == this.props.chooseStore.deviceNo) {
            check = true;
        }
        return <div style={{
            padding: '5px',
            ...
            <div style={{
                ...
                <div style={{ color: 'black', fontSize: '15px' }}>{item.deviceName}</div>
                <div style={{ fontSize: '13px', color: 'gray', marginTop: '8px' }}>{item.storeName}</div>
            </div>
        </div>
    }
